...
export function projectLimit(
  ...
  // Recent pace only escalates (max), never below the average floor, so it can
  // never *under*-warn. Used by the session window only; the weekly window
  // passes no recentPace (average-only — see callers).
