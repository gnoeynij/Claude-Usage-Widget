...
- **자유로운 조작 및 트레이 융합**:
  ...
  - `창 모서리 드래그`: 위젯 크기 자유 조절 (컨텐츠 반응형 스케일)
...
- **표시/가독성 커스터마이징**:
  ...
  - `진행바 눈금 개선`: 현재 세션 막대의 0/25/50/75/100 텍스트 잘림 현상 수정
- **최적화된 성능**:
...
3. **앱 조작**:
   ...
   - 앱 내 톱니바퀴 조작(⚙)으로 항상 위, 다크 모드, 언어(한국어/영어), 배경 투명도를 설정할 수 있습니다.
   - 설정 패널을 닫으면 위젯 높이가 열기 전 상태로 복원됩니다.
   - 위젯 사이즈는 설정이 아닌 창 모서리/가장자리 드래그로 조절합니다.
