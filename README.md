...
- **표시/가독성 커스터마이징**:
  ...
  - `SUIT SemiBold` 폰트 번들: 깔끔한 한글 가독성, 사용자 PC에 별도 설치 불필요
- **다중 PC 안전 (v1.3.0+)**:
  - 같은 Anthropic 계정을 여러 PC에서 사용해도 API에 부담 없이 동작하도록 설계
...
- **다중 PC API 안전성**: jitter (±10%) + 시작 지연 (0–2s) + 429 지수 백오프
- **자동 동기화 기본값**: 5분 → **10분** (공유 계정에서 유리)
...
- **코드 리팩토링**: 매직 넘버 → 모듈 상수, `_on_done` 분해, 중복 제거
