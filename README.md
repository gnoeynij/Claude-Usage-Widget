...
### v1.3.3 (현재)
- 시작 시 1회 자동 버전 체크 — 위젯 실행 후 약 1.5초 뒤, 옵션 패널 `Check for Updates` 버튼 옆에 `✓ 최신 버전` 또는 `● 새 버전 vX.Y.Z`가 표시됩니다 (네트워크 오류 시 무표시)
- 사용자가 직접 누르는 수동 체크 흐름은 그대로 유지

### v1.3.2
- 자동 업데이트 확인 / 다운로드 / 재시작 — 옵션 패널 하단의 `Check for Updates` 버튼으로 새 버전이 있으면 사용자에게 묻고, Windows `Downloads` 폴더에 다운로드한 뒤 자동 재시작
