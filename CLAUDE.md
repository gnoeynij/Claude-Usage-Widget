...
- **버전**: `v2.0.2` ([package.json](package.json), [package-lock.json](package-lock.json), [src-tauri/tauri.conf.json](src-tauri/tauri.conf.json), [src-tauri/Cargo.toml](src-tauri/Cargo.toml), [src-tauri/Cargo.lock](src-tauri/Cargo.lock), [src/state/store.ts](src/state/store.ts) 모두 동일 — 다음 bump 시 6곳 일괄 갱신, [BACKLOG](BACKLOG.md) P1).
- **빌드**: `npm run tauri build` exit 0. Windows MSVC 14.44.35207 + rustc 1.95.0 + Node 환경에서 51~87s. macOS Apple Silicon (rustc 1.95.0 + Node 20.20.2) 에서 19~31s. **CI 자동 빌드** ([.github/workflows/release.yml](.github/workflows/release.yml)) — tag push 시 양 OS runner 동시 (~5~10분).
...
- **디자이너** (UI·시각·UX·Liquid Glass·Mica·tokens.css·모드 전환·모달·tone): **컨텍스트 페르소나 — Apple 20년차 총괄 디자이너.** 현재 위젯의 Liquid Glass·Mica/Acrylic·tokens.css 체계가 이 정신으로 설계됨 — 신규 시각 결정은 *기존 디자인 언어와의 일관성*을 최우선으로 두고, 토큰 외 인라인 매직 넘버·과한 장식·실 .exe 캡처 미검증 변경 거부. UI 변경 SOP (§UI 변경 표준 절차) · `capture-widget.ps1` 실 .exe 캡처 · 회귀 사례 cross-check (§회귀 사례 기록) · `src/styles/tokens.css` 단일 출처 · Surgical changes
