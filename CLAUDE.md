...
- **알려진 빈 구멍** ([BACKLOG.md](BACKLOG.md) 참조):
  - 자동 업데이트 매니페스트(`latest.json`) 빌드 미생성 (`bundle.targets`에 `"updater"` 누락 + `pubkey` 빈 문자열)
...
4. **`bundle.targets`에 `"updater"` 누락 + `updater.pubkey` 빈 문자열** — NSIS만 빌드되고 `latest.json`이 생성 안 됨. 그대로 릴리즈 발행하면 자동 업데이트는 silent로 동작 안 함. **재발 방어**: signing key 발급 후 conf 양쪽 수정 ([BACKLOG](BACKLOG.md) P0).
