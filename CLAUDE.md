...
- **자동화 테스트**: 순수 함수 — Rust `cargo test` 36개 (pricing 22 + jsonl_aggregator 9 + usage_api 3 + device_sync 2) + TS `npm run test` Vitest 25개 (투영/pace 수학 `project.ts`). 양쪽 CI 게이트. UI·통합·시각 자동 테스트는 없음 — 빌드된 `.exe`/`.app` 실행 + UI 동작 + 캡처 스크립트로 수동 (`capture-widget.ps1` / `capture-widget.sh`).
- **자동 업데이트**: 양 OS 활성 (v2.4.10). `releases/latest/download/latest.json` endpoint, minisign 서명 검증. macOS 는 ad-hoc 서명 (Apple Developer 미가입), Gatekeeper 첫 실행 우회 README 안내.
