...
- **순수 함수 단위 테스트만 존재.** (1) Rust `cargo test` 36개: [`pricing.rs`](src-tauri/src/pricing.rs) 22 (`cost_usd`/`resolve`/`family_of` — 회귀 사례 §19 단가·1h cache·prefix 매칭·web search·inference_geo·fast mode·Sonnet 5 도입가·Opus 5 $0 누락 방어) + [`usage_api.rs`](src-tauri/src/usage_api.rs) 3 (응답 신필드 null 방어 + limits 배열) + [`jsonl_aggregator.rs`](src-tauri/src/jsonl_aggregator.rs) 9 (`group_blocks` 5h 경계·`active_view`·`overall_stats` cache_hit·`family_totals` 정렬·`recent_blocks` cap) + [`device_sync.rs`](src-tauri/src/device_sync.rs) 2. (2) **TS Vitest** `npm run test` 25개 ([`project.test.ts`](src/utils/project.test.ts)) — 투영/pace 순수 수학 ([`project.ts`](src/utils/project.ts) `projectLimit`/`blendPace`/`decayPace`): null 가드·평균 투영·recent 에스컬레이션·**floor(under-warn 불가)**·주간 0.1 ratio·anti-spike 가드·반감기 감쇠. 설정은 [`vitest.config.ts`](vitest.config.ts) (env node, app 플러그인 미로드 — 순수 TS 전용). **양쪽 모두 CI 게이트.** 그 외 (UI·통합·시각) 자동 테스트는 없음 — Playwright·컴포넌트 테스트 미구성. `period_totals` 는 `Local::now()` 의존이라 미테스트 (주입 리팩터 필요).
- 나머지 검증은 수동(빌드된 .exe 실행 → UI 동작 확인)으로 한다.
- Karpathy §4의 "테스트 작성 후 통과시키기" 패턴은 *순수 함수 영역* (Rust pricing/jsonl_aggregator + TS projection/pace)에만 적용 가능. **Vitest는 순수 수학에만 스코프 고정** — 컴포넌트/DOM/store 배선으로 번지지 말 것(stance: UI는 수동 유지). UI/시각 영역은 PR/커밋 시 *어떤 시나리오를 손으로 확인했는지* 텍스트로 명시 (예: "AOT toggle on/off, 옵션 패널 열고 닫기, sync 1회").
...
- **버전**: `v2.5.1` ([package.json](package.json), [package-lock.json](package-lock.json), [src-tauri/tauri.conf.json](src-tauri/tauri.conf.json), [src-tauri/Cargo.toml](src-tauri/Cargo.toml), [src-tauri/Cargo.lock](src-tauri/Cargo.lock), [src/state/store.ts](src/state/store.ts) 모두 동일. **bump 헬퍼** `node scripts/bump-version.mjs <semver>` (또는 `--check`) — 회귀 사례 §3 재발 방어. 2.5 = 계기판 테마 세대 명명 (v2.4.9~v2.4.10 회고 요약은 [docs/release-notes/v2.5.0.md](docs/release-notes/v2.5.0.md) 서두). v2.5.1 = macOS dmg CI corruption 패치(회귀 §23, **불충분으로 판명**) — v2.5.2 = 진짜 원인(코드 서명 부재) 패치 (회귀 사례 §24, 실기기 검증 대기).
...
- **자동화 테스트**: 순수 함수 — Rust `cargo test` 36개 (pricing 22 + jsonl_aggregator 9 + usage_api 3 + device_sync 2) + TS `npm run test` Vitest 33개 (투영/pace 수학 `project.ts` 25 + 알림 윈도우 지터 dedup `notifyWindow.ts` 8). 양쪽 CI 게이트. UI·통합·시각 자동 테스트는 없음 — 빌드된 `.exe`/`.app` 실행 + UI 동작 + 캡처 스크립트로 수동 (`capture-widget.ps1` / `capture-widget.sh`).
- **자동 업데이트**: 양 OS 활성 (v2.5.0). `releases/latest/download/latest.json` endpoint, minisign 서명 검증. macOS 는 ad-hoc 서명 (Apple Developer 미가입), Gatekeeper 첫 실행 우회 README 안내.
- **알려진 빈 구멍** ([BACKLOG.md](BACKLOG.md) 참조):
  - OAuth 토큰 **full refresh (B 방식 — refresh_token 으로 새 access token 발급)** 미구현·불필요 격하 — recovery (만료 banner + mtime polling + 자동 retry) 는 v2.0.0, **CLI spawn 자동 갱신 (C-변형)** 은 2026-07-03 완료: `TOKEN_EXPIRED` 시 위젯이 `claude -p` 최소 호출을 spawn 해 CLI 자체 refresh 를 트리거 (~$0.015/회, 에피소드당 1회 쿨다운), `NO_CREDENTIALS` 시 배너 버튼으로 `claude auth login` 터미널 오픈. [docs/plans/2026-05-20-oauth-refresh.md](docs/plans/2026-05-20-oauth-refresh.md) 부록 참조. B 방식 (직접 OAuth 호출) 은 spec 미공개·회귀 §21 체인 오염 위험으로 사실상 폐기.
  ...
  - macOS dmg 마운트 시 "손상됨" — v2.5.0/v2.5.1 모두 실사용자 설치 실패(2026-08-10). v2.5.1의 CI corruption 가설(회귀 §23)은 `--verbose` 로그로 반증(Finder 스킵 정상 적용 + hdiutil 압축 정상 완료인데도 동일 증상), `hdiutil imageinfo`로 파일 구조 자체는 멀쩡함도 확인 — 진짜 원인은 `signingIdentity` 미설정으로 .app이 번들 레벨 서명 자체가 없었던 것(Apple Silicon은 인터넷발 실행 파일에 서명 필수). v2.5.2에서 `signingIdentity: "-"` 추가로 조치, **실기기 재검증 전** — 회귀 사례 §24 참조.
...
| **opacity slider 영역** (`tokens.css --bg-alpha-mult` / `--scrim-bg` / `glass.css ::before` `::after` / `vibrancy_*.rs set_mica_enabled` / `store.ts scheduleCompositorRepaint`) | 6 layer 합성 (OS vibrancy · panel bg · panel ::before/::after · card bg · card ::before · Settings scrim) 중 한 layer 만 fade 시키면 다른 layer leak; macOS 는 alpha-only 변경 시 리페인트 skip (회귀 §16·§20) | ⚠ **캡처로 투명도 검증 불가** — `screencapture` 가 캡처 순간 webview 리페인트를 유발해, 라이브가 불투명(stale)이어도 이미지는 늘 투명하게 나옴 (회귀 §20). **투명도/opacity 는 사용자 라이브 화면(눈)이 유일 기준.** 캡처는 레이아웃·색·텍스트·scrim wash 까지만 (라이트/다크 비교도 사람 눈으로) |
...
19. **`pricing.rs` 단가 표 + `resolve()` prefix 매칭 — 모델 세대마다 동시 갱신 의무, prefix 비결정성도 함정** — Anthropic 이 모델 세대를 출시할 때마다 단가 그룹과 모델 ID 가 *6 곳 이상* 동시 변경. v2.1.4 시점 발견: (a) `claude-opus-4-7/4-6/4-5` 가 *Opus 4.x deprecated* 단가 ($15/$75) 로 잘못 계산 — 실 단가 $5/$25, *3배 과대*. (b) JSONL 의 `cache_creation.ephemeral_1h_input_tokens` / `ephemeral_5m_input_tokens` nested 객체가 *2025-08-13 1h cache GA 이후* 들어오는데 위젯이 *flat 합계만 읽어 모두 5m 단가 적용* — 1h cache 사용자에게 ~38% 과소. (c) `resolve()` prefix 매칭이 *HashMap iter 비결정 순서* 라 새 entry 추가 시 *date-suffix 모델이 잘못된 단가로 잡힐* 위험. **재발 방어**: (1) 신모델 출시 시 PRICING entry 추가 *+ 기존 entry 단가도 한 번 더 공식 표와 spot-check* (이번처럼 한 번 잘못 들어간 단가가 세 모델에 *복사 전파* 되는 흐름 차단). (2) 주석에 *공식 가격 URL + 마지막 검증일* 명시. (3) `resolve()` 는 *가장 긴 prefix 우선 + base 뒤가 끝/하이픈* 두 조건 함께 검사 — partial match (`claude-opus-4` 가 가상 `claude-opus-40-…` 와 매칭) 위험 영구 차단. (4) JSONL 신 필드 (`cache_creation` nested · `server_tool_use` · `service_tier` · `speed` · `inference_geo`) 가 *언제 도입되는지* 별도 추적 안 함 — *정기적 grep* 으로 missing 분기 발견. **검증**: brief 표 4 그룹 손계산 vs Rust 결과 매칭 + date-suffix 입력 (`claude-opus-4-7-20250416`) 이 `opus_current` 단가로 *결정적* 반환 확인. **사용자 비용 표시 직접 영향** — always-spot-check 의무.
20. **macOS WKWebView 가 isolated stacking context 의 alpha-only 변경을 리페인트 skip — opacity 변경이 모드 변경(리레이아웃) 전까지 시각 반영 안 됨** — `.glass-card` (+ `.glass-card::before` / `.glass-panel::before/::after`) 가 `isolation: isolate` 라 *isolated stacking context*. `setOpacity` 가 `--bg-alpha-mult` 만 바꾸면 background/`opacity` 의 *alpha 만* 변하는데, macOS WKWebView 가 그 합성 레이어를 캐시한 채 *리페인트를 skip*. 사용자는 슬라이더를 움직이거나 부팅해도 *불투명 그대로* 이고, **Mini/Normal/Detail 모드 변경(=창 리사이즈→리레이아웃)** 을 해야 투명도가 나타남. `LiquidGlass` 의 `el.offsetHeight` reflow 는 `.glass-panel` *본체 배경* 만 살림 — 카드 조상에 offsetHeight 가 걸려 있어도 *isolated 카드/pseudo 는 안 살아남* (offsetHeight=layout, 필요한 건 compositor repaint). 부팅도 동일: persisted opacity 가 *현재 모드 default 사이즈와 같으면* `applyModeSize` 가 실제 리사이즈를 안 해 리페인트도 안 일어남 → "업데이트 후 첫 실행에 불투명, 모드 바꿔야 적용" 의 진짜 원인 (boot *값* 적용 타이밍만 고친 fix 는 이걸 못 잡음). **fix** ([store.ts](src/state/store.ts) `scheduleCompositorRepaint`): `setOpacity` 가 `--bg-alpha-mult` 변경 후, 모드 변경과 *동일한* `set_window_size` 경로로 창을 1px 늘렸다 (rAF 후) 되돌려 compositor repaint 강제. 슬라이더 드래그 폭주 막게 ~90ms 디바운스, `IS_MAC` 한정 (Windows WebView2 repaint 경로 미검증 — 같은 버그일 수 있음). **재발 방어**: (a) `--bg-alpha-mult`·글래스 레이어 alpha 에 닿는 변경은 *compositor repaint 트리거 동반* 필수 — CSS 변수만 바꾸면 macOS 에서 안 보임. offsetHeight(layout)로는 isolated 레이어 안 살아남. (b) ⚠ **이 영역 검증은 캡처 금지** — `screencapture` 가 캡처 순간 리페인트를 유발(모드 변경과 같은 효과)해 *라이브 화면이 불투명(stale)이어도 캡처 이미지는 늘 투명하게 나옴*. 그래서 §16·표의 "6 컷 캡처" 가이던스는 *투명도엔 무효* — **사용자 라이브 화면(눈)이 유일 기준**. 이 세션에서 pre-sync 캡처로 fix 를 "증명" 했다고 한 게 무효였던 영역. (c) `isolation: isolate` 제거 검토 시 `::before` mask-composite specular 가 깨지므로 isolation 은 유지하고 repaint 트리거로 우회.
21. **`.credentials.json` 백업 복원 = rotating refresh token 체인 오염 → 전체 로그아웃** — Anthropic OAuth refresh token 은 *회전식 1회용*: refresh 성공 시 새 refresh token 이 발급되고, *구 토큰을 재사용하면 서버가 체인 전체를 폐기* (아직 만료 전인 access token 포함). 2026-07-03 CLI refresh 동작 실측 중 "테스트 후 원상복구" 로 백업 파일 (이미 소비된 refresh token 포함) 을 복원했다가 실제 로그아웃 사고 — 사용자 재로그인 1회로 해소. 부수 실측: `claude auth status` 는 refresh 안 함 (만료 토큰에도 `loggedIn: true` — 만료 감지 용도로도 못 씀), `claude -p` 실 API 호출만 refresh 트리거 (중립 cwd + `--tools ""` + `--no-session-persistence` 실측 $0.015/회·JSONL 오염 없음), `claude auth login` 은 시작 즉시 자격증명 wipe 후 브라우저 플로우 (중간 이탈 시 로그아웃 잔류 — *자동 호출 절대 금지*). **재발 방어**: (a) `.credentials.json` 은 영원히 read-only — 백업·복원·쓰기 금지, refresh 는 CLI spawn 에 위임. (b) 만료 시뮬레이션이 필요하면 *현재* 파일의 `expiresAt` 만 백데이트 (체인이 앞으로만 이동 — 안전), 절대 구 파일을 되돌리지 말 것. (c) 전체 실측 기록: [docs/plans/2026-05-20-oauth-refresh.md](docs/plans/2026-05-20-oauth-refresh.md) 2026-07-03 부록.

22. **새 window 라벨 생성 시 capabilities `windows` 배열 동반 의무** — v2.4.9까지 마일스톤 창($25k 이스터에그)이 [capabilities/default.json](src-tauri/capabilities/default.json) `windows: ["main","guide"]` 에 없어 창 내 *모든* IPC("I'm proud" close 버튼·ESC)가 ACL 로 silent 거부 — always-on-top + skip_taskbar 라 사용자가 닫을 수 없는 창이 됨 (v2.4.10 수정). §12(macOS start_dragging ACL)와 같은 뿌리 — §12 는 *permission* 누락, 이번엔 *window 라벨* 누락. guide 창은 배열에 있었고 milestone 만 빠져 "한 창은 되는데 한 창은 안 되는" 형태라 원인 특정이 늦어짐. **재발 방어**: (a) `WebviewWindowBuilder::new(&app, "<label>", …)` 로 새 라벨을 만들면 *같은 커밋*에서 capabilities `windows` 에 라벨 추가, (b) 새 창의 IPC 의존 흐름(최소 close 1회)을 실 .exe 에서 실측. 부수 함정: persist(`widget-settings.json`) 수기 편집 시 PowerShell 5.1 `Out-File utf8` 이 붙이는 BOM 을 serde_json 이 거부해 설정 전체 리셋 위험 — BOM-free 로 기록(`[System.IO.File]::WriteAllText` + `UTF8Encoding($false)`).

23. **macOS dmg 릴리즈 파이프라인이 CI 에서 조용히 손상된 dmg 를 만들 수 있음 — Finder AppleScript 자동화가 headless CI 미지원** (v2.5.1 `TAURI_BUNDLER_DMG_IGNORE_CI`, ⚠ 실기기 검증 대기) — v2.5.0 까지 macOS 실기기에서 직접 dmg 를 마운트해본 적이 없었는데(BACKLOG "macOS 실기기 검증" 미완료 항목과 동일 공백), 2026-08-10 사용자가 실제로 v2.5.0 `.dmg` 를 받아 열어보니 "손상되었기 때문에 열 수 없음 → 디스크 이미지 추출도 실패". 체크섬은 GitHub 자산과 일치(다운로드 손상 아님), CI 빌드 로그도 에러 없이 "성공"으로 끝남 — 즉 *CI 가 dmg 를 만드는 시점부터 이미 내부적으로 깨져 있었음*. 원인: Tauri 의 `bundle_dmg.sh` 가 설치 창(배경·아이콘 배치)을 꾸미려고 Finder 를 AppleScript 로 조작하는데, 이 자동화는 GUI 세션 없는 headless CI 에서 원래 지원 대상이 아님(Tauri 메인테이너 FabianLars 직접 확인: "the dmg bundle script isn't 'supposed' to work in CI" — [tauri-action#1003](https://github.com/tauri-apps/tauri-action/issues/1003)) — 에러 없이 "성공"으로 끝나면서도 조용히 깨진 압축 이미지를 만들 수 있음(동일 증상 [tauri-action#30](https://github.com/tauri-apps/tauri-action/issues/30), [#1091](https://github.com/tauri-apps/tauri-action/issues/1091)). **급할 때 우회**: 같은 릴리즈의 `.app.tar.gz`(업데이터용 산출물)는 dmg 를 안 거치므로 정상 — 다운로드 후 `tar -xzf` + `xattr -cr` 로 설치 가능. **fix** ([release.yml](.github/workflows/release.yml)): tauri-action env 에 `TAURI_BUNDLER_DMG_IGNORE_CI: false`(dmg 번들러가 CI 를 확실히 인식해 Finder 스타일링을 스킵하도록 강제 — 배경·아이콘 커스텀을 원래 안 썼으므로 잃는 것 없음) + `args: --verbose`(재발 시 `bundle_dmg.sh` 내부 로그 회수, §8 과 같은 로그-침묵 패턴 방어) 추가, v2.5.1 로 발행. **재발 방어**: (a) macOS 배포 산출물은 로컬 `cargo check`/`tauri dev` 통과와 *별개로* 실제 배포 경로(다운로드→마운트→실행)를 실기기에서 최소 1 회 왕복 검증해야 함 — CI "success" 는 파일이 유효하다는 보장이 아님. (b) dmg 배경·아이콘 커스텀이 필요해지면 `TAURI_BUNDLER_DMG_IGNORE_CI` 를 되돌리지 말고 별도 macOS self-hosted runner 또는 로컬 서명 빌드로 전환 검토(공식 GH runner 에서 Finder 자동화는 구조적으로 불안정하다고 메인테이너가 직접 인정). **⚠ 미검증** — v2.5.1 실제 릴리즈 후 사용자가 dmg 마운트를 재확인해야 이 항목이 확정됨; 실패 시 §17→§18 처럼 본 항목을 정정.

24. **§23 정정 — macOS "손상됨" 의 진짜 원인은 CI corruption 이 아니라 코드 서명 부재** (v2.5.2 `signingIdentity: "-"`, ⚠ 실기기 검증 대기) — v2.5.1 로 §23 fix(Finder AppleScript 스킵)를 발행했으나 사용자가 *동일 증상* 재현("손상됨" → 추출도 실패). `args: --verbose` 로 받은 실제 로그로 확인해보니 이번엔 Finder 스킵이 정말로 적용됐고("Will skip running AppleScript... because of --skip-jenkins option") hdiutil 압축도 전 파티션(MBR/GPT/HFS) CRC32 체크섬까지 에러 없이 완료 — §23 이 고친 메커니즘은 *이번엔 아예 발생하지 않았는데도* 같은 증상이 재현됨. 결정적 진단은 `hdiutil imageinfo`(순수 UDIF 컨테이너 구조 검사, Gatekeeper 와 무관) 를 다운로드한 파일에 직접 실행한 것 — 전 파티션 체크섬 일치 + 완전히 유효한 구조로 판정됨. 즉 *파일 바이트 자체는 처음부터 멀쩡*했는데도 Finder 더블클릭 마운트는 "손상됨" 을 띄운 것 — 이 괴리가 핵심 단서: `imageinfo` 는 Gatekeeper 를 안 거치지만, Finder/DiskImageMounter 의 더블클릭 마운트는 quarantine 플래그가 있는 파일에 대해 Gatekeeper 평가를 먼저 거친다. `tauri.conf.json` 의 `bundle.macOS.signingIdentity` 가 처음부터 미설정(default `null`) 이었고, CI 로그 전체를 grep 해도 `codesign` 호출이 *단 한 번도* 없었음 — .app 이 번들 레벨에서 전혀 서명되지 않은 채 배포되고 있었던 것. Apple Silicon 은 인터넷에서 받은 실행 파일에 서명을 요구하는데(Tauri 공식 문서: "code-signing is required for all apps from the Internet" on ARM), 서명이 아예 없으니 Gatekeeper 가 우회 가능한 "확인되지 않은 개발자" 경고 대신 우회 불가로 보이는 **"손상됨"** 을 띄운 것으로 추정 — §17 최초 조사 때 이미 스쳐간 가설([tauri-action#866](https://github.com/tauri-apps/tauri-action/issues/866): "ad-hoc 서명 없으면 damaged 뜬다")이 결국 맞았던 셈, 그때는 §23 의 CI-corruption 가설에 밀려 검증 없이 넘어갔음. **fix**: `tauri.conf.json` `bundle.macOS.signingIdentity: "-"` 추가 — 무료 ad-hoc 자가서명(Apple Developer 계정 불필요), 빌드 시 Tauri 가 `codesign --sign -` 를 자동 호출하도록 강제. **재발 방어**: (a) *체크섬 일치 + 압축 로그 정상* 은 "파일이 멀쩡하다"의 증거는 되지만 "Gatekeeper 도 통과한다"의 증거는 아님 — 컨테이너 무결성과 코드 서명 신뢰는 완전히 다른 레이어. "손상됨" 다이얼로그는 이름과 달리 코드 서명 문제의 대표 증상일 수 있다는 걸 재확인. (b) `hdiutil imageinfo`(Gatekeeper 무관, 순수 구조 검사) 를 이 클래스 문제의 *첫 진단 명령*으로 채택 — 통과하면 즉시 서명 쪽으로 의심 전환. (c) macOS 배포 산출물의 `signingIdentity` 설정 여부는 배포 전 필수 체크리스트 항목으로 격상 — CI 로그에 `codesign` 문자열 존재 여부 grep 한 줄로 확인 가능. (d) 여러 가설이 동시에 그럴듯할 때 *먼저 떠오른* 가설(CI corruption)에 고정되지 말고, 각 가설이 *반증 가능한 형태*로 예측을 내놓게 한 뒤(§23 은 "Finder 스킵하면 고쳐진다" 를 예측했고 실제로 검증 가능했음) 그 예측이 깨지면 즉시 다음 가설(서명)로 넘어갈 것 — 이번엔 `--verbose` 를 미리 심어둔 덕에 반증까지 한 사이클(하루 안)에 끝남. **⚠ 미검증** — v2.5.2 실제 릴리즈 후 사용자가 dmg 마운트를 재확인해야 이 항목이 확정됨.
...
- **`pricing.rs` 단가 표** (`src-tauri/src/pricing.rs`) — Anthropic 공식 가격 변동 직접 반영. 잘못된 단가는 사용자에게 표시되는 비용을 *수 배* 왜곡. 신모델 entry 추가 시 *기존 entry 도 함께 spot-check* (회귀 사례 §19).
