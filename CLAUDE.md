...
- **순수 함수 단위 테스트만 존재.** (1) Rust `cargo test` 23개: [`pricing.rs`](src-tauri/src/pricing.rs) 13 (`cost_usd`/`resolve`/`family_of` — 회귀 사례 §19 단가·1h cache·prefix 매칭·web search·inference_geo 방어) + [`usage_api.rs`](src-tauri/src/usage_api.rs) 2 (응답 신필드 null 방어) + [`jsonl_aggregator.rs`](src-tauri/src/jsonl_aggregator.rs) 8 (`group_blocks` 5h 경계·`active_view`·`overall_stats` cache_hit·`family_totals` 정렬·`recent_blocks` cap). (2) **TS Vitest** `npm run test` 20개 ([`project.test.ts`](src/utils/project.test.ts)) — 투영/pace 순수 수학 ([`project.ts`](src/utils/project.ts) `projectLimit`/`blendPace`/`decayPace`): null 가드·평균 투영·recent 에스컬레이션·**floor(under-warn 불가)**·주간 0.1 ratio·anti-spike 가드·반감기 감쇠. 설정은 [`vitest.config.ts`](vitest.config.ts) (env node, app 플러그인 미로드 — 순수 TS 전용). **양쪽 모두 CI 게이트.** 그 외 (UI·통합·시각) 자동 테스트는 없음 — Playwright·컴포넌트 테스트 미구성. `period_totals` 는 `Local::now()` 의존이라 미테스트 (주입 리팩터 필요).
...
- **자동화 테스트**: 순수 함수 — Rust `cargo test` 23개 (pricing 13 + jsonl_aggregator 8 + usage_api 2) + TS `npm run test` Vitest 20개 (투영/pace 수학 `project.ts`). 양쪽 CI 게이트. UI·통합·시각 자동 테스트는 없음 — 빌드된 `.exe`/`.app` 실행 + UI 동작 + 캡처 스크립트로 수동 (`capture-widget.ps1` / `capture-widget.sh`).
