...
| **6/23 Fable gating — usage 스키마 재확인 ✅ + extra_usage $X/$Y 금액 표시** | 인증·비용 | (2026-06-18 → 2026-06-25 실측) | 6/23 = Fable 5 가 Pro/Max/Team/Enterprise 플랜 한도에서 제외 → usage 크레딧(API 요율) 전환일. **✅ 스키마 재확인 done (2026-06-25 라이브 실측)**: 6/23 이후 usage 응답에 `seven_day_sonnet: null` + 블록별 신필드 `limit_dollars`/`used_dollars`/`remaining_dollars` 등장. sonnet null 이 non-Option struct 파싱을 깨 sync 실패 → `seven_day_sonnet` Option 방어 fix (✓ 완료 참조). **남은 것 ($X/$Y 표시) — 전체 구조 파악 done(2026-06-25), 구현 보류**: 금액 데이터는 `extra_usage`(monthly_limit/used_credits/currency/decimal_places) + 신규 `spend` 객체(used.amount_minor/limit/balance/can_purchase_credits)에 있음. 각 블록의 limit/used/remaining_dollars 는 구독 % 한도라 null. **현재 `extra_usage.is_enabled=false` + `spend.enabled=false` + used=$0 (사용자가 extra credit 미사용)이라 표시할 값 자체가 없음 → 구현은 크레딧 활성 시점으로 보류**(구조 파악됨, 그때 빠르게). 부수 발견(2026-06-25 실측): 신규 블록 다수(`seven_day_oauth_apps`/`cowork`/`omelette`/`tangelo`/`iguana_necktie`/`cinder_cove`/`amber_ladder` 등 코드네임·실험) + `limits` 배열 + `spend` 객체 — 전부 null·위젯이 무시(안전). pricing.rs 는 6/18·6/25 공식 가격표와 전 항목 일치(변경 불필요). 메모리 `oauth-usage-new-fields` 참조. |
...
| 항목 | 영역 | 출처 | 비고 |
|---|---|---|---|
| ~~**fast mode 비용 분기**~~ ✓ **완료 (2026-06-10, `947ebae`)** | 비용 정확도 | [회귀 §19](CLAUDE.md) | `usage.speed=="fast"` → `resolve_fast()`: Opus 4.8 $10/$50(2x)·4.6/4.7 $30/$150(6x), 캐시는 fast base input 파생(1.25x/2x/0.1x — 공식 문서 "multipliers stack" 확인). Fable/Sonnet/Haiku 는 fast 미지원 → 표준 fallback($0 방지). cargo test 4종. *주의: 과거 메모의 "speed 는 record 최상위" 는 부정확 — 실측 30일 JSONL 에서 `message.usage.speed` 로 확인되어 usage 에서 읽음.* Mythos 5 단가($10/$50=Fable 티어)도 같은 커밋. |
| **모델별 전체-기간 합계 복원** | Detail UX | (2026-06-10 Detail 재설계) | v2.4 Detail 재설계에서 Models 카드(전체-기간 family 합계: 비용+토큰, 예: Opus $7,344/2.9B)를 일별 추세 차트로 흡수하며 **의식적으로 제거** — 차트는 최근 7/30일 믹스만 보여줌. 사용자가 전체-기간 모델 합계를 다시 원하면: Lifetime 카드에 family 미니 스택바 1줄 또는 footer 캡션으로 복원. 데이터는 `by_family`(aggregate)에 그대로 있어 UI 만 붙이면 됨. |
...
| ~~**`CLAUDE_CODE_OAUTH_TOKEN` env var fallback**~~ **DEAD (2026-06-02)** | UX·인증 | (Claude Desktop App 호환 시도) | **구현해도 작동 불가**: `claude setup-token` 의 `sk-ant-oat01-*` 토큰이 ~2026-02-20 부터 API 거부 + Anthropic 이 서드파티 앱의 Free/Pro/Max OAuth 토큰 재사용을 ToS 명시 금지("OpenClaw ban", [claude-code#28091](https://github.com/anthropics/claude-code/issues/28091)). ⚠ 위젯 *현재* 방식(Claude Code OAuth 토큰 → `/api/oauth/usage`)도 같은 ToS 회색지대 — usage 엔드포인트가 아직 차단 안 됐을 뿐, 인증 모델이 정책 리스크에 노출. 서드파티 권장은 Console API key (구독 OAuth 아님). |
...
| **v2.2.0 — 누적 비용(기기별·통합 합산) + 실시간 카운트다운 + 인앱 사용량 알림** | tag `v2.2.0` | 2026-06-08 |
| **v2.3.0 — 단일 인스턴스 락 + Fable 5 단가 + 토스트 중복 fix** | tag `v2.3.0` | 2026-06-10 |
| **v2.3.1 — 비용 누락 방지(누적·일별 히스토리) + 알림/AOT fix + fast 단가** | tag `v2.3.1` | 2026-06-10 |
| **v2.4.0~v2.4.3 — Detail 재설계 라인** (Models 카드 → 일별 추세 차트 흡수, P3 "모델별 전체-기간 합계 복원" 참조) + 후속 fix. release notes 미작성 — 상세는 `git log v2.3.1..v2.4.3`. | tag `v2.4.0`~`v2.4.3` | 2026-06-12~19 |
