...
- **Detail** (592×619, minSize 520×520) — 대시보드: 활성 세션(현재 5h 블록 — 비용 · 남은 시간 · 시간당 비용), 일별 비용 추세 차트(모델별 색, 7 / 14 / 30일 범위, 막대 클릭으로 날짜 선택, 기기 토글), 합계 카드(이번 주 / 달 · 누적 · 전체 기기 합산)

푸터 SegmentedControl 또는 트레이 메뉴로 전환. 각 모드는 default size + minSize가 따로 있고, 드래그로 조정한 크기는 *모드별로 기억*됩니다.

### 한도 투영
도넛과 주간 막대에 반투명 **고스트 아크·도트**가 겹쳐, 현재 속도가 유지될 때 초기화 시점의 예상 사용량을 표시하고 초기화 캡션에 예상치가 함께 나옵니다. 한도 도달이 예상되면 Mini에 **⚠ 위험 배지**(클릭 시 한도별 상세)가 뜨고 백그라운드에서 **트레이 알림**이 발생합니다. 최근 사용 폭주를 함께 반영해 경고가 더 일찍 나타나며, 초기화 직후 잠시(데이터가 쌓일 때까지)는 표시하지 않습니다.
