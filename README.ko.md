...
- **Normal** (320×360) — Donut hero + 주간 capsule: 전체 모델과 플랜이 추적하는 모델별 한도를 서버 라벨 그대로 표시(예: Max의 **Fable** — Anthropic이 대상을 바꿔도 자동 추종). 기본 모드
...
### 우클릭 빠른 메뉴
위젯 아무 곳이나 — Windows에서 원래 시스템 메뉴가 뜨던 드래그 영역까지 포함해 — 우클릭하면 빠른 액션이 열립니다: **사용량 요약 한 줄 복사**(세션/주간/스코프 % + 오늘 비용), 지금 동기화, 모드 전환, 숨기기, 로그 폴더 열기, 그리고 위젯이 마우스를 통과시켜 뒤 창을 그대로 조작할 수 있는 **클릭 통과 모드**(해제는 트레이 우클릭 → "클릭 통과 해제").
...
- **종료** — 트레이 우클릭 → `Quit`
- **설정** — 헤더 `⚙` 버튼
- **우클릭** — 위젯 아무 곳: 사용량 요약 복사·지금 동기화·모드 전환·클릭 통과·숨기기·로그 폴더 열기
...
- [**v2.4.8**](docs/release-notes/v2.4.8.md) — 주간 영역이 새 `limits` 배열로 공식 앱과 같은 정보를 표시: 플랜의 스코프 한도(현재 Max의 Fable)가 서버 라벨 그대로 별도 행으로 렌더되고 대상 모델이 바뀌어도 자동 추종, 은퇴한 "Sonnet only" 행이 무의미한 0%로 남지 않음 + 우클릭 메뉴 전면 추가(사용량 요약 복사·동기화·모드 전환·클릭 통과(트레이 해제)·숨기기·로그 폴더) — Windows 드래그 영역의 시스템 메뉴도 대체 + 설정 패널 스크롤바가 카드 모서리를 관통하던 문제 수정.
