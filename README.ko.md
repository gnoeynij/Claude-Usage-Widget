...
**Claude Code**의 **Anthropic API 사용량**을 한눈에 보여주는 데스크탑 위젯 — 현재 세션, 주간 한도, 최근 블록, 모델별 비용까지. 브라우저나 콘솔 없이 화면 한 구석에서 확인. **Tauri 2 + SolidJS + Rust** 스택으로 처음부터 다시 만든 **Windows** 전용 빌드.
...
시스템 backdrop과 OS-level vibrancy를 합성. 배경 투명도 슬라이더는 *배경만* fade되고 텍스트·donut·게이지는 항상 불투명.
...
### OAuth 토큰 자동 회복
`~/.claude/.credentials.json` mtime을 polling 해서 Claude Code CLI가 토큰을 갱신하면 다음 sync 주기 기다리지 않고 즉시 retry. 만료 시 위젯 내 banner로 `claude` 실행 안내 + 자동 복구.
...
3. **실행** — 실행에는 PC에 [Claude Code](https://docs.anthropic.com/en/docs/agents-and-tools/claude-code/overview)가 1회 이상 로그인된 인증 정보(`~/.claude/.credentials.json`)가 필요합니다.
4. **조작**
...
사전 요구: Node ≥ 20, Rust 툴체인(`rustup`), Windows의 경우 Microsoft C++ Build Tools ("Desktop development with C++" 워크로드). Windows 11에는 WebView2 런타임이 기본 탑재되어 있고, Windows 10에서는 인스톨러 부트스트래퍼가 자동으로 받아옵니다.
...
> NSIS 인스톨러가 권장 배포 형식입니다. Windows 10에서 WebView2를 자동 설치하고, `tauri-plugin-updater`의 silent in-place 업데이트와 연동되기 때문입니다.
