...
- **Detail** (592×619, minSize 520×520) — 4-카드 대시보드: 활성 세션, 기간별(오늘/어제/이번주/이번달), 최근 5h 블록, 모델별 사용량
...
### 사용량 알림
- 현재 5시간 세션 한도 *또는* 7일 주간 한도의 **85%** · **95%** 도달 시 OS 알림 발송
- 블록당 임계치별 1회만 발송 — 매 sync 반복 X
- 알림 권한은 첫 임계치 도달 시점에 lazy 요청 (부팅 시 X). 표시 여부는 OS 설정에 따름
